...
**Чего стенд по-прежнему не переживёт:** потери узла вместе с его диском.
Тома `local-path` привязаны к машине, поэтому журнал смен на время аварии
становится недоступен и ждёт возвращения узла. Это следующий пункт плана.
...
1. Хранилище, не привязанное к одному диску: пока тома `local-path` держат
   журнал и мониторинг на своём узле и ждут его возвращения.

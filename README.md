...
| [Учения третьего протокола](#учение-обрыв-40-секунд-у-обоих-парков-сразу) | обрыв на два парка сразу, четыре попытки подлога, проверка в кластере |
...
### Третий протокол в кластере, а не только в compose

Всё, что выше, снято на локальном стенде. В кластере то же самое проверено
отдельно, потому что между ними есть разница, которая на площадке и решает:
там между бортом и шлюзом стоит `Service`, сетевая политика и выкат через
GitOps, а не общая сеть Docker.

```
cluster/apps/25-egts.yaml               Service с портом 7779/UDP и порт в поде
cluster/apps/30-quarry.yaml             парк balanced на TRANSPORT=statement
cluster/apps/50-network-policies.yaml   вход: UDP 7779 от подов с меткой part: sim
cluster/apps/51-network-policies-egress.yaml   выход бортов на тот же порт
```

**Учение с обрывом, кластер:** буфер парка на EGTS вырос до 5060 записей и
после восстановления дал задержку приёма **29,1 с**; парк на заявлениях
удержался на **2,3 с**, выбросив 926 протухших отсчётов и отчитавшись о них.
Задержка в кластере в покое около 1 секунды против 0,23 на локальном стенде:
это цена сети подов и трёх машин на одном ноутбуке, а не протокола.

**Учение с подлогом, кластер:** те же четыре попытки, те же четыре причины
отказа, ни одна не попала в показатели.

**Учение с посторонним подом:** под без метки `part: sim` в том же
пространстве имён отправил пять дейтаграмм на UDP 7779. Отправка прошла без
единой ошибки сокета, а на шлюзе не появилось ни одной ошибки разбора: сетевая
политика их не пропустила. Это стоит запомнить отдельно: с TCP посторонний
получил бы отказ в соединении и понял, что его не пускают. С UDP он не узнаёт
ничего, как не узнаёт и борт, у которого сменился адрес приёмника.

### Грабли, которые нашёл только кластер

- 🔴 **Образ сменился, манифест нет, и ArgoCD разницы не видит.** У приёмника
  поменялся код, а в манифесте не изменилось ни байта: приложение осталось
  `Synced`, под продолжил работать на старом образе, и подпись не проверялась
  вовсе. Тег `:local` и `imagePullPolicy: Always` тут не помогают, они решают
  другую половину задачи: новый образ приезжает при СОЗДАНИИ пода, а создавать
  его никто не просил. Лечится `kubectl rollout restart` после каждой доставки
  образов, и это надо делать привычкой, а не вспоминать по симптомам.
- 🔴 **Пара хранилища после перезагрузки осталась без главной, и это встало
  выкатом.** Машины поднялись раньше арбитра, обе стороны DRBD остались
  `Secondary`, NFS не отдавался. Реестр контура живёт на этом же хранилище,
  поэтому `docker pull` внутри кластера не падал с ошибкой, а **висел**:
  двенадцать минут `ContainerCreating` без единого события. Порядок важен:
  сначала `cluster/storage/arbitr.sh start`, потом машины хранилища. Лечится
  перезапуском keepalived на держателе общего адреса.
- 🔴 **Отставший приёмник выбрасывает подписанную телеметрию целиком.** После
  переключения парка приёмник разбирал хвост старых сообщений и отставал на
  166 секунд. Всё подписанное, что он читал в это время, было старше своего
  срока годности: 1176 отсчётов в `срок годности вышел` и ни одного принятого,
  пока он не догнал. На локальном стенде та же грабля стоила 306 отсчётов, в
  кластере с большим хвостом она выглядит как «протокол не работает». Работает,
  просто он отказывается считать данные, которые уже никому не нужны.
...
| Под висит `ContainerCreating` без событий, образ не тянется | хранилище пары без главной, реестр не отдаёт слои: `cluster/storage/nas-pair.sh status`, затем перезапуск keepalived на держателе адреса |
| Образ доставлен, ArgoCD `Synced`, а работает старый код | манифест не менялся, пода никто не пересоздавал: `kubectl -n quarry rollout restart deploy/<имя>` |
| После переключения парка подписанное не принимается совсем | приёмник отстал и читает то, что уже старше срока годности: дождаться, пока догонит |

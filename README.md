...
egts/               протокол EGTS по ГОСТ: кодек, приёмный шлюз на два протокола
...
## Часть пятая: борт говорит по EGTS и по Wialon IPS
...
один раз, на одном переходе, и потому могло оказаться совпадением. Теперь
протокола два, и работают они одновременно: **парк `fixed` говорит по EGTS,
парк `balanced` по Wialon IPS**. В журнале смены их записи неразличимы.
...
**Две машины это минимум, а не идеал.** При разрыве связи между ними каждая
может решить, что вторая умерла; на площадке от этого ставят третий голос или
ограждение по питанию. В стенде принят простой ответ: главной остаётся та, что
удержала общий адрес.
...
1. Третий голос для пары хранилища, чтобы разрыв связи между машинами
   разбирался без человека.
2. Ingress и постоянные имена вместо проброса портов: на площадке в контур

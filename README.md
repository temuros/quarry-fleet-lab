...
| Событие | Когда |
|---|---|
| Питание вернули, API кластера ответил | через **54 секунды** |
| Все три узла `Ready` | через **55 секунд** |
| Вся нагрузка вернулась | через **97 секунд** |
...
целиком. Починено в `cluster/apps/20-kafka.yaml`, проверено повторным
обесточиванием: смещения продолжаются с прежних значений.
...
1. Реестр образов, переживающий потерю узла, и хранилище, не привязанное к
   одному диску: сейчас это единственная точка отказа контура.
2. Ansible вместо скриптов установки и алерты поверх мониторинга.
3. Сетевые политики внутри кластера и второй управляющий узел: сейчас

...
настолько, насколько его переживает реестр. На площадке реестр либо выносят
за пределы кластера, либо держат в нескольких экземплярах с хранилищем,
не привязанным к одному узлу.
...
задачи, устройства часов от гипервизора у машин нет, и `timedatectl` честно
пишет `System clock synchronized: no`. Метку времени событию ставит тот узел,
на котором в этот момент работает приёмник, поэтому расхождение часов едет
прямо в отчёт по смене. Источник времени внутри периметра это не украшение,
а часть требований к такой площадке; в стенде он следующим пунктом плана.
...
**Чего стенд не переживёт:** потери управляющего узла. Он один, и это следующий
пункт плана.
...
1. Источник времени внутри периметра: без него часы узлов расходятся, а с ними
   едет отчёт по смене. Это показала учебная авария.
2. Реестр образов, переживающий потерю узла, и хранилище, не привязанное к
   одному диску: сейчас это единственная точка отказа контура.
3. Ansible вместо скриптов установки и алерты поверх мониторинга.
4. Сетевые политики внутри кластера и второй управляющий узел: сейчас
   управляющий один, и его потеря останавливает управление кластером.
5. Приём второго протокола борта (Wialon IPS) тем же шлюзом: на площадке техника

...
#
# ⚠️ Настройка переехала в роль `cluster/ansible/roles/vremya`, и там она
# идёт по всему стенду, а не только по узлам кластера: этот скрипт берёт
# список из `kubectl get nodes`, поэтому машины хранилища оставались без
# часов вообще. Здесь остаётся быстрый осмотр `check`.
set -euo pipefail

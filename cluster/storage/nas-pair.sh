...
#
# ⚠️ Установка и правила переезда переехали в роль `cluster/ansible/roles/
# hranilishche`, и она единственный источник правды: скрипты переключения и
# конфигурация keepalived раскладываются оттуда. Здесь остаются осмотр
# (`status`) и разовый перенос данных. Правку, сделанную только тут, следующий
# прогон роли сотрёт.
set -euo pipefail
